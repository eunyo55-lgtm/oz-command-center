...
import {
  mockJourneyFunnel, mockRFMDistribution, mockTrendData
} from '../lib/mockData';
...
import { useSignals, useKPI } from '../hooks/useData';
import {
...
export default function CommandPage() {
  ...
  const { data: signals, source: signalsSource, error: signalsError } = useSignals();
  ...
  const signalsByDept = useMemo(() => {
    ...
    signals.filter(s => s.status !== 'resolved' && s.status !== 'dismissed').forEach(s => {
      counts[s.department]++;
    });
    return counts;
  }, [signals]);
      ...
      <section className="px-8 pt-8 pb-6 border-b border-paper-300">
          ...
          <div className="hidden lg:flex items-center gap-6 shrink-0">
            ...
            <div className="text-right">
              ...
              <div className="text-4xl font-bold text-success numeric tracking-tight">
                {kpi.resolved_signals_7d}
              </div>
            </div>
          </div>
        </div>

        {/* Date range filter + connection status */}
        <div className="flex items-center justify-between mb-6 gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            <span className="micro-label">분석 기간</span>
            <ConnectionStatus source={signalsSource} error={signalsError} />
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <KPICard
            ...
            label="2차 구매율"
            subLabel="신규 회원"
            value={kpi.new_signup_repurchase_rate}
            target={kpi.target_repurchase_rate}
            unit="%"
            accentColor="#FFB000"
          />
          <KPICard
            ...
            label="앱 매출 비중"
            subLabel="전체 매출 대비"
            value={kpi.app_revenue_share}
            target={kpi.target_app_revenue_share}
            unit="%"
            accentColor="#5878E8"
          />
          <KPICard
            ...
            label="총 고객"
            subLabel="누적 회원"
            value={kpi.total_customers / 10000}
            target={null}
            unit="만명"
            accentColor="#00A87D"
          />
